...
function setupEventListeners() {
  ...
  // Reload sessions explicitly on sidebar refresh button
...
function renderSourcesDropdown() {
  elements.selectSource.innerHTML = `
    <option value="">None / Scratchpad (Start without Repo)</option>
  `;
...
function handleSourceChange() {
  ...
  if (!selectedName) {
    elements.branchContainer.classList.add('hidden');
    return;
  }

  elements.branchContainer.classList.remove('hidden');
...
function renderSessionsList() {
  ...
  filtered.forEach(session => {
    ...
    const sourceLabel = session.sourceContext?.source
...
function renderSessionHeader(session) {
  ...
  elements.sessionSourceText.innerText = session.sourceContext?.source || 'Scratchpad (No Repo)';
...
async function handleDownloadZip() {
  ...
  const btn = elements.btnDownloadZip;
  btn.disabled = true;
  btn.innerHTML = `<i data-lucide="loader" class="w-4 h-4 animate-spin"></i><span>Packaging...</span>`;
  lucide.createIcons();
  ...
  try {
    ...
    const zip = new JSZip();
    const session = state.activeSession || state.sessionCache[sessionId] || {};
    const activities = state.activitiesCache[sessionId] || [];

    // Add session.json
    zip.file('session.json', JSON.stringify(session, null, 2));

    // Add summary.md
    let summaryMd = `# Session: ${session.title || 'Untitled'}\n\n`;
    summaryMd += `- **Session ID:** ${sessionId}\n`;
    summaryMd += `- **Prompt:** ${session.prompt || 'N/A'}\n`;
    summaryMd += `- **State:** ${session.state || 'N/A'}\n`;
    summaryMd += `- **Source:** ${session.sourceContext?.source || 'Scratchpad (No Repo)'}\n\n`;
    summaryMd += `## Activity Transcript\n\n`;

    let patchCounter = 1;

    activities.forEach((act, i) => {
      const time = act.createTime ? new Date(act.createTime).toLocaleString() : `Step ${i + 1}`;
      const orig = act.originator || 'system';

      if (act.userMessaged) {
        summaryMd += `### [${time}] User\n${act.userMessaged.userMessage}\n\n`;
      } else if (act.agentMessaged) {
        summaryMd += `### [${time}] Jules\n${act.agentMessaged.agentMessage}\n\n`;
      } else if (act.planGenerated && act.planGenerated.plan) {
        summaryMd += `### [${time}] Plan Generated\n`;
        (act.planGenerated.plan.steps || []).forEach((st, idx) => {
          summaryMd += `${idx + 1}. **${st.title}**: ${st.description || ''}\n`;
        });
        summaryMd += `\n`;
      } else if (act.progressUpdated) {
        summaryMd += `### [${time}] Progress: ${act.progressUpdated.title || ''}\n${act.progressUpdated.description || ''}\n\n`;
      }

      // Extract patches as .patch files inside patches/ folder
      if (act.artifacts && act.artifacts.length > 0) {
        act.artifacts.forEach(art => {
          if (art.changeSet && art.changeSet.gitPatch) {
            ...
            const patchFilename = `patches/change_${patchCounter++}.patch`;
            const patchContent = `# Commit Message: ${patch.suggestedCommitMessage || 'Code change'}\n# Base Commit: ${patch.baseCommitId || 'head'}\n\n${patch.unidiffPatch || ''}`;
            zip.file(patchFilename, patchContent);
            summaryMd += `> Saved Git Patch to \`${patchFilename}\`\n\n`;
          }
          if (art.bashOutput) {
            zip.file(`logs/bash_${i + 1}.log`, `$ ${art.bashOutput.command || ''}\nExit Code: ${art.bashOutput.exitCode}\n\n${art.bashOutput.output || ''}`);
          }
        });
      }
    });
    ...
    a.href = blobUrl;
    a.download = `jules-session-${sessionId.substring(0, 8)}.zip`;
    ...
    alert(`Failed to create Zip download: ${err.message}`);
  } finally {
    btn.disabled = false;
    btn.innerHTML = `<i data-lucide="download" class="w-4 h-4"></i><span>Download Zip</span>`;
    lucide.createIcons();
  }
}
